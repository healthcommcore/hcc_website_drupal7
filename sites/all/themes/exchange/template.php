...
// Hack to add description, keyworkds to homepage even if it has no content 
	// or node associated with it
 ...
 */
	if (drupal_is_front_page()) {
		...
		$description = array(
			...
			'#attributes' => array(
				'name' => 'description',
				'content' => $variables['page']['content']['metatags']['global:frontpage']['description']['#attached']['drupal_add_html_head'][0][0]['#value']
			),
		);
		$keywords = array(
			'#tag' => 'meta',
...
function exchange_form_element($variables) {
...
/**
 * Check which if any child menu items are currently active
 */
/*
 */
	...
	if (is_array($items)) {
		foreach ($items as $item) {
			...
			if ( isset($item['#below']) && count($item['#below']) > 0) {
				hasActiveChild($item['#below']);
			}
			 */
			if ( isset($item['#href']) && $_GET['q'] == $item['#href']) {
				return true;
			}
		}
	}
...
/*
		 Here we want to add the active-trail class to parent item to keep it the 
		 style active even when its child is active. This is supposed to be a 
		 default behavior in Drupal but for some reason it's overriden somewhere 
		 in this Exchange theme. I couldn't find where so I wrote this theme 
		 function to bring it back
		  -Dave Rothfarb 11-13-14
 */
 ...
 */
	...
	if ( isset($element['#attributes']['class']) && in_array('expanded', $element['#attributes']['class']) && count($element['#below'] > 0) && $element['#original_link']['menu_name'] == 'main-menu' ){
		if ( hasActiveChild($element['#below']) ){
